...
export async function getStatsLuxuryCard(username, preloadedStats = null) {
  ...
  statItems.forEach((item, index) => {
    ...
    const x = column === 0 ? 45 : 250;
    const y = yOffset + row * 58;
    ...
    statsHTML += `
      ...
      <text class="stat-value" x="${x}" y="${y + 28}">${item.value}</text>
  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
      ...
      <!-- Rank section -->
      <circle cx="420" cy="${rankY}" r="55" fill="none" stroke="url(#goldGrad)" stroke-width="2"/>
